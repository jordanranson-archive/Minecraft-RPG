...
public class EnchantmentHelper
{
	...
	public static int getFrozenWeaponModifier(InventoryPlayer par0InventoryPlayer, EntityLiving par1EntityLiving)
    {
        return getEnchantmentLevel(Enchantment.frozenWeapon.effectId, par0InventoryPlayer.getCurrentItem());
    }
	
	public static int getLifestealWeaponModifier(InventoryPlayer par0InventoryPlayer, EntityLiving par1EntityLiving)
    {
        return getEnchantmentLevel(Enchantment.lifestealWeapon.effectId, par0InventoryPlayer.getCurrentItem());
    }
	
	public static int getFleeWeaponModifier(InventoryPlayer par0InventoryPlayer, EntityLiving par1EntityLiving)
    {
    ...
    {
        List var3 = buildEnchantmentList(par0Random, par1ItemStack, 0, par2);
    ...
    public static List buildEnchantmentList(Random par0Random, ItemStack par1ItemStack, int effectId, int par2)
    {
        ...
        else
        {
			...
			// add elemental enchantments here
			if (effectId != 0)
			{
				var8 = new ArrayList();
				...
				{
					for (int j = Enchantment.fireAspect.getMinLevel(); j <= Enchantment.fireAspect.getMaxLevel(); ++j) // for each level of an enchantment
					{
						if(var7 >= Enchantment.fireAspect.getMinEnchantability(j) && var7 <= Enchantment.fireAspect.getMaxEnchantability(j))
						{
							level = j;
						}
					}
					...
					{
						newEnchant = new EnchantmentData(Enchantment.fireAspect, level);
					}
					else if(effectId == EnumMagicEffect.freeze.getEffectId())
					{
						newEnchant = new EnchantmentData(Enchantment.frozenWeapon, level);
					}
					else if(effectId == EnumMagicEffect.lifesteal.getEffectId())
					{
						newEnchant = new EnchantmentData(Enchantment.lifestealWeapon, level);
					}
					else if(effectId == EnumMagicEffect.flee.getEffectId())
					{
						newEnchant = new EnchantmentData(Enchantment.fleeWeapon, level);
					}
				}
				else 
				{
					if(effectId == EnumMagicEffect.flame.getEffectId())
					{
						newEnchant = new EnchantmentData(Enchantment.flame, 1);
					}
					else if(effectId == EnumMagicEffect.freeze.getEffectId())
					{
						newEnchant = new EnchantmentData(Enchantment.frozen, 1);
					}
					else if(effectId == EnumMagicEffect.lifesteal.getEffectId())
					{
						newEnchant = new EnchantmentData(Enchantment.lifesteal, 1);
					}
					else if(effectId == EnumMagicEffect.flee.getEffectId())
					{
						newEnchant = new EnchantmentData(Enchantment.flee, 1);
					}
				}
            ...
            {
                EnchantmentData var10 = (EnchantmentData)WeightedRandom.getRandomItem(par0Random, var9.values()); // weighted list of enchantments
					
                if (var10 != null) // has weighted list
                {
					if(var8 == null)
					{
						var8 = new ArrayList();
                    ...
                    for (int var11 = var7; par0Random.nextInt(50) <= var11; var11 >>= 1)
                    {
                        ...
                        while (var12.hasNext()) // while we have enchantments in the list
                        {
     ...
     * passed.
     */
    public static Map mapEnchantmentData(int enchantability, ItemStack itemStack)
    {
		...
		excludedEnchantments[3] = Enchantment.frozenWeapon.effectId;
		excludedEnchantments[4] = Enchantment.lifesteal.effectId;
        ...
        {
            Enchantment enchantment = enchantmentList[i];

            if (enchantment != null && enchantment.type.canEnchantItem(item)) // can this item be enchanted?
            {
                for (int j = enchantment.getMinLevel(); j <= enchantment.getMaxLevel(); ++j) // for each level of an enchantment
                {
					// if its within range of each level, it adds it to the hash map, so if you can afford
					// 3 levels of efficiency, for example, then it will add effic. levels 1, 2, and 3 to 
					// the map. 
                    if (enchantability >= enchantment.getMinEnchantability(j) && enchantability <= enchantment.getMaxEnchantability(j)) // and enchantability is within range
                    {
                        if (hashMap == null)
                        {
                            hashMap = new HashMap();
						...
						for(int k = 0; k < excludedEnchantments.length; k++)
						{
							if(excludedEnchantments[k] == enchantment.effectId)
							{
								canAdd = false;
							}
						}
						...
						{
							// add the enchantment
							hashMap.put(Integer.valueOf(enchantment.effectId), new EnchantmentData(enchantment, j));
						}
                    }
                }
            }
