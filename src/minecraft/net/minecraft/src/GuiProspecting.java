...
public class GuiProspecting extends GuiContainer
{
    /** The book model used on the GUI. */
    private static ModelBook bookModel = new ModelBook();
    private Random rand = new Random();

    /** ContainerProspecting object associated with this gui */
    private ContainerProspecting ContainerProspecting;
    ...
    public GuiProspecting(InventoryPlayer par1InventoryPlayer, World par2World, int par3, int par4, int par5)
    {
        super(new ContainerProspecting(par1InventoryPlayer, par2World, par3, par4, par5));
        this.ContainerProspecting = (ContainerProspecting)this.inventorySlots;
    }
    ...
    {
        this.fontRenderer.drawString(StatCollector.translateToLocal("container.prospect"), 12, 6, 4210752);
    ...
    protected void mouseClicked(int par1, int par2, int par3)
    {
		...
		int clientX = par1 - var5;
		int clientY = par2 - var6;
		
		if (clientX >= 75 && clientX < 75 + 22 && clientY >= 30 && clientY < 30 + 22)
		{
			this.ContainerProspecting.setLootResults();
			if(this.ContainerProspecting.prospectItem(this.ContainerProspecting.lootList[0], this.ContainerProspecting.lootList[1]))
			{
				this.mc.playerController.sendProspectPacket(this.ContainerProspecting.windowId, this.ContainerProspecting.lootList[0], this.ContainerProspecting.lootList[1]);
			}
		}
    }
    ...
    {
        int var4 = this.mc.renderEngine.getTexture("/gui/prospecting.png");
		...
		this.ContainerProspecting.onCraftMatrixChanged(this.ContainerProspecting.craftMatrix);
		if(this.ContainerProspecting.getProspect())
		{
			if(x >= 75 && x < 75 + 26 && y >= 30 && y < 30 + 26) // mouse hover
			{
				this.drawTexturedModalRect(var5 + 74, var6 + 29, 176, 28, 28, 28);
			}
			else
			{
				this.drawTexturedModalRect(var5 + 74, var6 + 29, 176, 0, 28, 28);
			}
		}
        ...
        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        this.mc.renderEngine.bindTexture(var4);
        EnchantmentNameParts.instance.setRandSeed(this.ContainerProspecting.nameSeed);*/
        ...
        /*for (int var13 = 0; var13 < 3; ++var13)
        {
            ...
            this.zLevel = 0.0F;
            this.mc.renderEngine.bindTexture(var4);
            int var15 = this.ContainerProspecting.enchantLevels[var13];
            GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public void func_74205_h() // animation when book is open, probably the letters
    {
        ...
        {
            if (this.ContainerProspecting.enchantLevels[var3] != 0)
            {
                var2 = true;
            }
        }
